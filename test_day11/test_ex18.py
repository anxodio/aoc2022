...
class Monkey:
    ...
    def inspect_next_item(self) -> ItemMovement:
        ...
        target = (
            self.true_target
            if (new_item % self.divisible_test) == 0
            else self.false_target
        )
        self.inspection_count += 1
        return ItemMovement(target, new_item)
...
def get_monkey_business_level(monkeys: list[Monkey], rounds: int = 20) -> int:
    # I just searched this math property in the solutions :/
    boring_product = 1
    for monkey in monkeys:
        boring_product *= monkey.divisible_test

    for i in range(rounds):
        for monkey in monkeys:
            while monkey.items:
                movement = monkey.inspect_next_item()
                monkeys[movement.monkey_id].items.append(
                    movement.worry_level % boring_product
                )
...
def test_item_inspection():
    monkey = _get_test_monkeys()[0]
    assert monkey.items == [79, 98]
    assert monkey.inspect_next_item() == ItemMovement(3, 1501)
...
def test_get_monkey_business_level():
    ...
    assert get_monkey_business_level(monkeys) == 10197


def test_get_monkey_large_business_level():
    monkeys = _get_test_monkeys()
    assert get_monkey_business_level(monkeys, 10000) == 2713310158
...
if __name__ == "__main__":
    ...
    print(get_monkey_business_level(monkeys, 10000))
